...
function showSpiner(isLoading) {
    isLoading ? spiner.style.display = 'block' : spiner.style.display = 'none';
}
...
function startGame(){
    ...
    let counterNegative = document.querySelector('.counterNegative');
    ...
    for (let i = 0; i < questions.length; i++){
        btn[i].addEventListener('click', () => {
            if (questions[i].answer.toLowerCase() === input[i].value.toLowerCase()) {
                ...
                counterPos++;
                counterPositive.innerHTML = counterPos;
                ...
                counterNeg++;
                counterNegative.innerHTML = counterNeg;
            }
        })
    }
}
